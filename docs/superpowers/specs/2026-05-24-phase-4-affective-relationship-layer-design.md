...
- 新增 `ResponseStrategy`，综合 persona contract、affect state、relationship state 和 task context，生成 compact policy hint。
...
2. `AffectState` 是短期分析结果，必须带 `confidence` 和 evidence summary。
3. `RelationshipState` 是互动基线，来自 Phase 3 memory，但不把 memory 当成绝对心理事实。
4. `ResponseStrategy` 是策略，不是剧情；它只告诉 agent 如何回应。
5. Persona Contract 不能被普通 run 自动更新。重大修改必须显式确认、版本化并写 changelog。
6. Analyzer 失败不能阻塞用户回答。
7. Web UI 展示用于审计，不做情绪化装饰。
8. affect 结果可以影响回答风格，但不能绕过安全、权限或工具 gate。
9. affect 结果不能反向污染 Phase 3 memory；后续若要写 memory，必须走 Phase 3 validator。
10. 所有持久化路径必须留在 `.cyrene/` 下，拒绝路径穿越和 symlink 写入。
...
response-strategy.ts
  把 persona contract、affect state、relationship state 和 task context 编译成 policy hint。
...
`events.jsonl` 保存每次 strategy 生成的摘要事件，只写 labels、confidence、strategy 和 rationale 摘要，不保存完整用户原文。
...
- Avoid: claimed sentience, romantic attachment, therapeutic diagnosis, emotional manipulation
...
- compiled policy prompt 不包含 claimed sentience、romantic attachment 或 therapeutic diagnosis wording。
...
- `continuity` 数据结构包含 memory、affect、relationship 和 response strategy。
...
[ ] continuity 在 memory 下展示 affect / relationship / strategy
...
4. 实现 response strategy compiler。
5. 把 `affect-runtime` 接入 Web / CLI / REPL prompt 构建。
6. 写 `.cyrene/affect/state.json` 和 `events.jsonl`。
7. 把 Web 右栏 memory 区域升级为 `continuity`。
8. 补齐 unit / integration / regression tests。
