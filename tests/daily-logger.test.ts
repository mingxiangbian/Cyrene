...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cyrene-daily-'))
  tempDirs.push(dir)
  return dir
}
...
describe('appendDaily', () => {
  ...
  it('refuses to append through a symlinked .cyrene directory', async () => {
    const root = await createTempDir()
    ...
    await expect(readFile(join(outsideCyrene, 'memory', 'daily.md'), 'utf8')).rejects.toMatchObject({
      code: 'ENOENT'
    })
  })
})
