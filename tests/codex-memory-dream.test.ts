...
import { runCodexMemoryDream } from '../src/codex/memory-dream.js'
...
describe('Codex memory dream runtime', () => {
  ...
  it('deep does not promote same-run duplicate evidence', async () => {
    const home = await createTempDir('cyrene-dream-home-')
    vi.stubEnv('HOME', home)
    const cwd = await createTempDir('cyrene-dream-project-')
    const candidate = createPending({
      seenCount: 2,
      evidence: [
        { runId: 'same-run', evidenceGroupId: 'same-group', summary: 'First.' },
        { runId: 'same-run', evidenceGroupId: 'same-group', summary: 'Duplicate.' }
      ]
    })
    const memoryRoot = await seedProjectPending(cwd, [candidate])
